...
A test for significance of the random effects will be based on
the chisquare distribution with 2 degrees of freedom, comparing the
...
The control based refinement was not able to reliable
...
adequately when: family sizes are large modest to large,
...
a reservation that extends to any data set modeled with a random effect per
subject but having only one observation per subject. 
