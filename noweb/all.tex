...
\nwhypn{coxme}=
 coxme <- function(formula,  data, 
     ...
     \nwhypf{process-standard-arguments1}{process-standard-arguments}{process-standard-arguments2}
     \nwhypf{decompose-formula1}{decompose-formula}{decompose-formula2}
     \nwhypf{build-control-structures1}{build-control-structures}{build-control-structures2}
     \nwhypf{call-computation-routine1}{call-computation-routine}{call-computation-routine2}
     \nwhypf{finish-up1}{finish-up}{finish-up2}
 \}
\end{nwchunk}
...
\begin{nwchunk}
\nwhyp{process-standard-arguments2}{process-standard-arguments}{process-standard-arguments1}{process-standard-arguments3}=
 if (!missing(fixed)) \{
     if (missing(formula)) \{
         formula <- fixed
...
\begin{nwchunk}
\nwhyp{process-standard-arguments3}{process-standard-arguments}{process-standard-arguments2}{process-standard-arguments4}=
...
\begin{nwchunk}
\nwhypb{process-standard-arguments4}{process-standard-arguments}{process-standard-arguments3}=
...
parentheses.
\begin{nwchunk}
\nwhypb{decompose-formula2}{decompose-formula}{decompose-formula1}=
...
coxmeFull.
If there is only one random term then \Verb!varlist! is
...
\begin{nwchunk}
\nwhyp{build-control-structures2}{build-control-structures}{build-control-structures1}{build-control-structures3}=
...
\begin{nwchunk}
\nwhyp{build-control-structures3}{build-control-structures}{build-control-structures2}{build-control-structures4}=
 \nwhypf{get-cmat1}{get-cmat}{get-cmat2}
 \nwhypf{get-groups1}{get-groups}{get-groups2}
 \nwhypf{make-vinit1}{make-vinit}{make-vinit2}
 \nwhypf{newzmat1}{newzmat}{newzmat2}
...
\begin{nwchunk}
\nwhypb{make-vinit2}{make-vinit}{make-vinit1}=
...
managable chunks.
\begin{nwchunk}
\nwhypb{call-computation-routine2}{call-computation-routine}{call-computation-routine1}=
...
(Is this needed?)
\begin{nwchunk}
\nwhyp{finish-up2}{finish-up}{finish-up1}{finish-up3}=
 if (is.character(fit)) \{
     fit <- list(fail=fit)
     oldClass(fit) <- 'coxme'
     return(fit)
...
historical label.
\begin{nwchunk}
\nwhyp{finish-up3}{finish-up}{finish-up2}{finish-up4}=
...
\begin{nwchunk}
\nwhyp{finish-up4}{finish-up}{finish-up3}{finish-up5}=
...
\begin{nwchunk}
\nwhyp{finish-up5}{finish-up}{finish-up4}{finish-up6}=
...
\begin{nwchunk}
\nwhypb{finish-up6}{finish-up}{finish-up5}=
...
and slash operators.
\begin{nwchunk}
\nwhypb{get-groups2}{get-groups}{get-groups1}=
...
\begin{nwchunk}
\nwhypb{get-cmat2}{get-cmat}{get-cmat1}=
...
\begin{nwchunk}
\nwhypb{newzmat2}{newzmat}{newzmat1}=
...
not surprisingly \Verb!formula1! and \Verb?formula2?.  
...
the formula \Verb!y ~ x1 + x2*(x3 + x4)!.
...
The \Verb!formula1! routine is called with the model formula,
the response and the fixed parts are returned as the \Verb!fixed!
...
\begin{nwchunk}
\nwhypf{formula1}{formula}{formula2}=
 formula1 <- function(x) \{
...
\begin{nwchunk}
\nwhyp{formula2}{formula}{formula1}{formula3}=
...
\begin{nwchunk}
\nwhyp{formula3}{formula}{formula2}{formula4}=
     if (class(x)== 'call' && x[[1]] == as.name('-')) \{
         temp1 <- formula1(x[[2]])
...
\begin{nwchunk}
\nwhyp{formula4}{formula}{formula3}{formula5}=
...
\begin{nwchunk}
\nwhyp{formula5}{formula}{formula4}{formula6}=
...
\begin{nwchunk}
\nwhyp{formula7}{formula}{formula6}{formula8}=
     return(list(fixed=x))
 \}
\end{nwchunk}
...
\begin{nwchunk}
\nwhyp{formula8}{formula}{formula7}{formula9}=
...
\begin{nwchunk}
\nwhyp{formula9}{formula}{formula8}{formula10}=
...
\begin{nwchunk}
\nwhyp{formula10}{formula}{formula9}{formula11}=
 hasAbar <- function(x) \{
   if (class(x)== 'call') \{
...
a single argument.
\begin{nwchunk}
\nwhypb{formula11}{formula}{formula10}=
 subbar <- function(x) \{
...
\begin{nwchunk}
\nwhypf{initmatch1}{initmatch}{initmatch2}=
...
\nwhypn{coxmeFull}=
 coxmeFull <- function(collapse=FALSE) \{
     ...
     #  initialize function
     \nwhypf{coxmeFull-init1}{coxmeFull-init}{coxmeFull-init2}
     \nwhypf{coxmeFull-generate1}{coxmeFull-generate}{coxmeFull-generate2}
     \nwhypf{coxmeFull-wrapup1}{coxmeFull-wrapup}{coxmeFull-wrapup2}
...
\begin{nwchunk}
\nwhypb{coxmeFull-init2}{coxmeFull-init}{coxmeFull-init1}=
 initialize <- function(vinit, fixed, intercept, G, X,  control) \{
     ...
     \nwhypb{initmatch2}{initmatch}{initmatch1}
     
     if (ngroup==0) \{
         if (intercept)
             return(list(error=("Invalid null random term (1|1)")))
         else \{
             \nwhypf{coxmeFull-init-11}{coxmeFull-init-1}{coxmeFull-init-12}
             \}
         \}
     else \{
         if (nvar==0) \{
             \nwhypf{initialize-inits1}{initialize-inits}{initialize-inits2}
             \}
 
         # Deal with random slope terms
         if (ngroup ==1) \{
             \nwhypf{coxmeFull-init-21}{coxmeFull-init-2}{coxmeFull-init-22}
                 \}
         else \{
             if (collapse) \{
                 \nwhypf{coxmeFull-init-3b1}{coxmeFull-init-3b}{coxmeFull-init-3b2}
                 \}
             else \{
                 \nwhypf{coxmeFull-init-3a1}{coxmeFull-init-3a}{coxmeFull-init-3a2}
                 \}
             \}
 
         #Deal with slopes
         if (nvar > 0) \{
             \nwhypf{coxmeFull-init-41}{coxmeFull-init-4}{coxmeFull-init-42}
             \}
         \}
     \}
\end{nwchunk}
...
shrinkage term such as \Verb!(x1 + x2 | 1)!.
In this case the two coefficients for \Verb!x1! and \Verb?x2? are considered
...
\begin{nwchunk}
\nwhypb{coxmeFull-init-12}{coxmeFull-init-1}{coxmeFull-init-11}=
 xname <- dimnames(X)[[2]]
 if (length(vinit) >0) \{
...
36 is close to \Verb!-log(.Machine$double.eps)!, used in the Splus care.exp
...
\begin{nwchunk}
\nwhypb{coxmeFull-generate2}{coxmeFull-generate}{coxmeFull-generate1}=
 generate= function(newtheta, parms) \{
     ...
     \nwhypf{coxmeFull-generate-21}{coxmeFull-generate-2}{coxmeFull-generate-22}
     \nwhypf{coxmeFull-generate-31}{coxmeFull-generate-3}{coxmeFull-generate-32}
     \nwhypf{coxmeFull-generate-41}{coxmeFull-generate-4}{coxmeFull-generate-42}
     \}
\end{nwchunk}
...
\begin{nwchunk}
\nwhyp{coxmeFull-init-22}{coxmeFull-init-2}{coxmeFull-init-21}{coxmeFull-init-23}=
...
\begin{nwchunk}
\nwhypb{coxmeFull-init-23}{coxmeFull-init-2}{coxmeFull-init-22}=
 varlist <- list(vmat)
 if (nvar==0) 
...
\begin{nwchunk}
\nwhypb{coxmeFull-generate-22}{coxmeFull-generate-2}{coxmeFull-generate-21}=
 
 if (parms$case==2) return(theta*parms$varlist[[1]])
\end{nwchunk}

Matching any user input for either
the \Verb!vfixed! or \Verb?vinit? arguments (which show up here as
\Verb!fixed] and [[vinit!) for cases 2 and 3 can be
...
\begin{nwchunk}
\nwhypb{initialize-inits2}{initialize-inits}{initialize-inits1}=
 gname <-  names(G)
 ntheta <- length(gname)
...
\begin{nwchunk}
\nwhyp{coxmeFull-init-3a2}{coxmeFull-init-3a}{coxmeFull-init-3a1}{coxmeFull-init-3a3}=
...
what we need.
\begin{nwchunk}
\nwhypb{coxmeFull-init-3a3}{coxmeFull-init-3a}{coxmeFull-init-3a2}=
...
of times.
\begin{nwchunk}
\nwhypb{coxmeFull-generate-32}{coxmeFull-generate-3}{coxmeFull-generate-31}=
 if (parms$case==3) \{
...
\begin{nwchunk}
\nwhypb{coxmeFull-init-3b2}{coxmeFull-init-3b}{coxmeFull-init-3b1}=
...
\begin{nwchunk}
\nwhyp{coxmeFull-init-42}{coxmeFull-init-4}{coxmeFull-init-41}{coxmeFull-init-43}=
 xvar  <- apply(X,2,var)
...
\begin{nwchunk}
\nwhypb{coxmeFull-init-43}{coxmeFull-init-4}{coxmeFull-init-42}=
...
\begin{nwchunk}
\nwhypb{coxmeFull-generate-42}{coxmeFull-generate-4}{coxmeFull-generate-41}=
 if (parms$case==4) \{
...
\begin{nwchunk}
\nwhyp{coxmeFull-wrapup2}{coxmeFull-wrapup}{coxmeFull-wrapup1}{coxmeFull-wrapup3}=
...
\begin{nwchunk}
\nwhyp{coxmeFull-wrapup3}{coxmeFull-wrapup}{coxmeFull-wrapup2}{coxmeFull-wrapup4}=
     if (parms$case==3) \{
...
% $b$ by $1/s^2$.
\begin{nwchunk}
\nwhypb{coxmeFull-wrapup4}{coxmeFull-wrapup}{coxmeFull-wrapup3}=
     if (parms$case==4) \{
...
\nwhypn{coxmeMlist}=
 coxmeMlist <- function(varlist, rescale=FALSE, pdcheck=TRUE,  positive=TRUE) \{
     ...
     positive <- positive
     \nwhypf{coxmeMlist-init1}{coxmeMlist-init}{coxmeMlist-init2}
     \nwhypf{coxmeMlist-generate1}{coxmeMlist-generate}{coxmeMlist-generate2}
     \nwhypf{coxmeMlist-wrapup1}{coxmeMlist-wrapup}{coxmeMlist-wrapup2}
     out <- list(initialize=initialize, generate=generate, wrapup=wrapup)
     class(out) <- 'coxmevar'
     out
     \}
...
\begin{nwchunk}
\nwhypb{coxmeMlist-init2}{coxmeMlist-init}{coxmeMlist-init1}=
 initialize <- function(vinit, fixed, intercept, G, X, control) \{
     ngroup <- min(length(G), ncol(G))
     nvar   <- min(length(X), ncol(X))  # a NULL or a nx0 matrix yields 0
     if (ngroup >0 & nvar >0)
     ...
     \nwhypf{Mlist-initial-value1}{Mlist-initial-value}{Mlist-initial-value2}
     \nwhypf{Mlist-matrix-checks1}{Mlist-matrix-checks}{Mlist-matrix-checks2}
...
\begin{nwchunk}
\nwhypb{Mlist-initial-value2}{Mlist-initial-value}{Mlist-initial-value1}=
...
\begin{nwchunk}
\nwhypb{Mlist-matrix-checks2}{Mlist-matrix-checks}{Mlist-matrix-checks1}=
 for (j in 1:ntheta) \{
     kmat <- tlist[[j]]
     if (rescale) \{
         temp <- diag(kmat)
...
\begin{nwchunk}
\nwhypb{coxmeMlist-generate2}{coxmeMlist-generate}{coxmeMlist-generate1}=
  generate <- function(newtheta, parms) \{
      theta <- parms$theta
...
\begin{nwchunk}
\nwhypb{coxmeMlist-wrapup2}{coxmeMlist-wrapup}{coxmeMlist-wrapup1}=
...
\begin{nwchunk}
\nwhypn{coxme.fit}=
 \nwhypf{coxme-setup1}{coxme-setup}{coxme-setup2}
     \nwhypf{null-fit1}{null-fit}{null-fit2}
     \nwhypf{define-penalty1}{define-penalty}{define-penalty2}
     \nwhypf{coxfit6a-call1}{coxfit6a-call}{coxfit6a-call2}
     \nwhypf{coxme-fit1}{coxme-fit}{coxme-fit2}
     \nwhypf{coxme-finish1}{coxme-finish}{coxme-finish2}
 \}
\end{nwchunk}
...
element.
\begin{nwchunk}
\nwhyp{define-penalty2}{define-penalty}{define-penalty1}{define-penalty3}=
...
matrix \Verb!tmat!; if that contains a substantial number of sparse
columns then it is important to subset \emph{before} creating a
regular matrix from the remainder; the construction
\Verb!(as.matrix(tmat))[k,k]! would create a temporary matrix of possibly
vast proportions.  
At this point in time all of the intercepts map before any covariates
so we can keep separate indices for the intercept-rows-so-far \Verb!indx1! and
covariate-rows-so-far \Verb!indx2!, with the second one starting after the
end of all the intercepts.  
What we are doing is in essence a diagonal bind of matrices, pasting
blocks down the diagonal, but S has no \Verb!dbind! function.
\begin{nwchunk}
\nwhyp{define-penalty3}{define-penalty}{define-penalty2}{define-penalty4}=
...
\begin{nwchunk}
\nwhypb{coxfit6a-call2}{coxfit6a-call}{coxfit6a-call1}=
...
\begin{nwchunk}
\nwhypf{define-logfun1}{define-logfun}{define-logfun2}=
...
\end{description}
\begin{nwchunk}
\nwhyp{coxme-setup2}{coxme-setup}{coxme-setup1}{coxme-setup3}=
...
\begin{nwchunk}
\nwhypb{coxme-setup3}{coxme-setup}{coxme-setup2}=
     if (ncol(y) ==3) \{
         if (length(strata) ==0) \{
...
a null model.
\begin{nwchunk}
\nwhypb{null-fit2}{null-fit}{null-fit1}=
 if (is.null(ifixed) ) \{
     ifixed <- rep(0., ncol(x))
...
\begin{nwchunk}
\nwhypb{coxme-fit4}{coxme-fit}{coxme-fit3}=
 gkmat <- gchol(kfun(theta, varlist, vparm, ntheta, ncoef))
 if (is.variance) \{
     ikmat <- solve(gkmat)  #inverse of kmat, which is the penalty
     fit <- .C(ofile,
...
\begin{nwchunk}
\nwhypb{coxme-finish2}{coxme-finish}{coxme-finish1}=
 \nwhypf{coxme-lastvar1}{coxme-lastvar}{coxme-lastvar2}
 \nwhypf{coxme-rescale1}{coxme-rescale}{coxme-rescale2}
 \nwhypf{coxme-df1}{coxme-df}{coxme-df2}
 \nwhypb{refine2}{refine}{refine1}
 .C("coxfit6e", as.integer(ncol(y)))  #release memory
 \nwhypf{create-output-list1}{create-output-list}{create-output-list2}
\end{nwchunk}
...
estimates.
\begin{nwchunk}
\nwhypb{coxme-lastvar2}{coxme-lastvar}{coxme-lastvar1}=
...
\begin{nwchunk}
\nwhyp{coxme-rescale2}{coxme-rescale}{coxme-rescale1}{coxme-rescale3}=
     if (nvar2 ==0) \{
...
\begin{nwchunk}
\nwhypb{coxme-rescale3}{coxme-rescale}{coxme-rescale2}=
     else \{
...
sum.  
\begin{nwchunk}
\nwhypb{coxme-df2}{coxme-df}{coxme-df1}=
...
\begin{nwchunk}
\nwhypb{create-output-list2}{create-output-list}{create-output-list1}=
 idf <- nvar + sum(ntheta)
...
\nwhypn{lmekin}=
 lmekin <- function(formula,  data, 
     ...
     \nwhypf{lme-process-standard-arguments1}{lme-process-standard-arguments}{lme-process-standard-arguments2}
     \nwhypf{decompose-lme-formula1}{decompose-lme-formula}{decompose-lme-formula2}
     \nwhypb{build-control-structures4}{build-control-structures}{build-control-structures3}
     \nwhypf{lmekin-compute1}{lmekin-compute}{lmekin-compute2}
     \nwhypf{lmekin-finish-up1}{lmekin-finish-up}{lmekin-finish-up2}
     \}
 \nwhypf{lmekin-helper1}{lmekin-helper}{lmekin-helper2}
\end{nwchunk}
...
\begin{nwchunk}
\nwhypb{lme-process-standard-arguments2}{lme-process-standard-arguments}{lme-process-standard-arguments1}=
 if (!missing(fixed)) \{
     if (missing(formula)) \{
         formula <- fixed
...
\begin{nwchunk}
\nwhypb{decompose-lme-formula2}{decompose-lme-formula}{decompose-lme-formula1}=
 flist <- formula1(formula)
 if (hasAbar(flist$fixed))
     stop("Invalid formula: a '|' outside of a valid random effects term")
...
\begin{nwchunk}
\nwhyp{lmekin-compute2}{lmekin-compute}{lmekin-compute1}{lmekin-compute3}=
 \nwhypb{define-penalty4}{define-penalty}{define-penalty3}
 \nwhypf{define-xz1}{define-xz}{define-xz2}
 \nwhypf{lmekin-fit1}{lmekin-fit}{lmekin-fit2}
\end{nwchunk}
...
\begin{nwchunk}
\nwhypb{define-xz2}{define-xz}{define-xz1}=
 #Define Z^* and X^*
...
\begin{nwchunk}
\nwhypb{lmekin-fit2}{lmekin-fit}{lmekin-fit1}=
 mydiag <- function(x) \{
...
be modest. 
\begin{nwchunk}
\nwhypb{lmekin-compute3}{lmekin-compute}{lmekin-compute2}=
 vmat <-  kfun(theta, varlist, vparm, ntheta, ncoef)
 Delta <- t(solve(chol(as(vmat, "dsCMatrix"), pivot=FALSE)))
 zstar <- rBind(zstar1, Delta)
 qr1 <- qr(zstar)
...
\begin{nwchunk}
\nwhyp{lmekin-finish-up2}{lmekin-finish-up}{lmekin-finish-up1}{lmekin-finish-up3}=
...
\begin{nwchunk}
\nwhypb{lmekin-finish-up3}{lmekin-finish-up}{lmekin-finish-up2}=
...
\begin{nwchunk}
\nwhypb{lmekin-helper2}{lmekin-helper}{lmekin-helper1}=
...
\begin{nwchunk}
\nwhypf{bdsmatrix1}{bdsmatrix}{bdsmatrix2}=
...
object does not.
\begin{nwchunk}
\nwhyp{bdsmatrix2}{bdsmatrix}{bdsmatrix1}{bdsmatrix3}=
...
the same.
\begin{nwchunk}
\nwhyp{bdsmatrix3}{bdsmatrix}{bdsmatrix2}{bdsmatrix4}=
...
\begin{nwchunk}
\nwhypb{bdsmatrix4}{bdsmatrix}{bdsmatrix3}=
