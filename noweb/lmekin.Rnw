...
function [[kfun]] which returns $\Delta$ given the parameters $\theta$.
...
Remember that for the genetic problems lmekin was
designed for $Z$ will be very large and sparse while $X$ will
be modest. 
