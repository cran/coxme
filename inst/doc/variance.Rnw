...
\begin{itemize}
  ...
  \item vfixed: a vector of fixed values for the parameters, as supplied
    ...
    is fixed at 0.4.
  \item intercept: Signals that the term include random intercepts per
...
have a search list of length 1.
...
Since the parial log-likelihood (PL) depends only on the linear predictor
...
However, occassionally the maximizer makes a very large step in the
...
than about .0001 has a neglibible effect on the linear predictor.
