...
%\VignetteIndexEntry{Special Variance Stuctures}
%\VignetteDependes{surviva}
...
\begin{align}
  ...
  b &= N(0, \sigma^2 R) \\
  R_{ij} &= \begin{cases} 1 & \text{if } i=j\\
                                  \rho & \text{otherwise} 
				  \end{cases}
\end{align}
...
This is equivalent to a shrinkage method found commonly in the 
the utilzation of questionairres, where all
...
\begin{itemize}
  ...
  \item fixed: a vector of fixed values for the parameters, as supplied
    by the user.  For instance adding the clause 
    \texttt{variance=list(rho=.4)} could be used to fit a model where $\rho$
    is fixed at 0.4.
  \item intecept: Does the term include an intercept.  This would be true
    for a term like \texttt{(1+ trt | site)}, but is invalid 
    for this variance function.
...
<<>>=
geinit <- function(vinit, fixed, intercept, G, X, sparse) {
    if (length(G) >0)  
    ...
    if (length(fixed) >0) {
        indx <- match(names(fixed), c("sigma", "rho"))
        if (any(is.na(indx))) 
            stop("Unrecognized parameter name in vfixed values")
        if (is.list(fixed)) {
            if (any(sapply(fixed, length) !=1))
...
The next block of statements reads in any initial values for the parameters.
The parameter vector as a whole is $\theta=(\sigma^2, \rho)$.
If no initial values were given then a default grid of values for the
iniitial search is $\sigma=.05, .2, .6$ and $\rho= .01, .5, .9$.
Note that the final $\theta$ is always a list, even if all the elements
parameters only have length 1
...
\end{align*}
The actual optimization is done by the \texttt{optim} routine, and need 
...
\texttt{imap} of 1, 7, 4, \ldots would map subject 1 to coefficent 1, 
subject 7 to coeffient 2, subject 3 to coefficient 4, etc.
A valid imap value will be a matrix with one column per grouping variable
(the term (1|teacher/school) would have 2 columns), each row contains the
...
The input arguements to this function are the current vector of
iterated parameters, along with the list of parameters supplied by the
init function.
The result of the routine is the covariance matrix $\Sigma$ of the
random effects for the term.

One of the greatest challenges for this routine is to guarrantee that the
resulting matrix is positive definite to a sufficient degree that the
creation of $\Sigma^{-1}$ will give a full rank result.
The Affymetric gene-set for which this routine was developed had
values between 6 and 15, which in turn means that any coefficient less
than about .0001 has an effect size of effectively zero.  
Very small variances, which lead to $b$ vectors of that size, need not 
be persued to a bitter end of $\log(\epsilon) \rightarrow -\infty$
for $\epsilon \rightarrow 0$.  For many gene sets
$\sigma=0$ is the actual MLE, but the change in LL for these smallest
values becomes so tiny as to approach machine precision.
The second limit in the routine came from a particular data set where
an intermediate guess in the maximization gave too large a variance,
far beyond what is biologically feasable, which in turn led to
...
@ 
This routine ingores any vinit or vfixed arguments and does absolutely
