...
and [[varlist]] became [[variance]]. 
This set of names makes a lot mores sense for the user.  
However, we immediately rename `variance' to `varlist'; in writing this
intertwined code/document it is just too confusing to use the same word
for both a statisical concept (variance of a random effect) and for
a program object which can be many things.
